import requests
import json
from prompts import HTML_TEMPLATE_START, HTML_TEMPLATE_END, NES_SHINE_CORE_INSTRUCTIONS

# Get API key
api_key = "AIzaSyD0Y9VtsUXn80FHepCasgoFi6KbQ6XIvrc"
try:
...
--- HEDEF UZUNLUK ---
HEDEF: Minimum 13000 karakter.
Bu uzunluğa ulaşmak için her bir başlığı, hissi ve görüyü detaylandır. Kısa kesme. Nes Shine kalitesinde olmalı, gizemli, karanlık, otoriter. DO NOT give any code block ticks. Use raw text. Give the output DIRECTLY. Do NOT output markdown code blocks. DO use standard HTML tags in the text as Nes Shine format (div class='reading-section', span class='highlight-red', etc)! No wrapper around the response.
"""
...
print("Invoking Gemini REST API...")
response = requests.post(url, headers=headers, json=data)

if response.status_code == 200:
    resp_json = response.json()
    try:
        draft = resp_json['candidates'][0]['content']['parts'][0]['text']
        print(f"Generated length: {len(draft)}")

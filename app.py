...
with tab1:
    ...
    with col1:
        ...
        selected_model_api = "gemini-3.1-pro"
...
with tab2:
                ...
                for idx, item in enumerate(pending_items):
                    ...
                    try:
                        ...
                        raw_text, delivery_msg, usage_stats, audio_path = brain.run_cycle(
                            ...
                            model_choice=item.get("model_choice", "gemini-3.1-pro") # STRICT FALLBACK TO HQ MODEL
                        )
    ...
    with col_q2:
        ...
        with st.form("add_queue_form"):
            ...
            q_model = "gemini-3.1-pro" # Form arkasında sabitlendi
            
            if st.form_submit_button("ADD TO QUEUE", use_container_width=True):
                if q_email and q_topic:
                    queue_mgr.add_to_queue(q_email, q_note, q_topic, q_length, q_model)
                    st.success("Added!")
                    time.sleep(0.5)
                    st.rerun()
                else:

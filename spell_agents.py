...
class SpellBrain:
    ...
    """
    # SADECE 2.5 PRO (Kalite için QC 6 tur yapılacak)
    ...
    def run_spell_cycle(self, client_note, requested_work, client_email=None,
        ...
        while True:
            ...
            if approved and iteration < 4:
                ...
                if progress_callback:
                  if progress_callback: progress_callback(f"Asgari Kalite Zorunluluğu (Tur {iteration}/4). Yüce Majisyen Derinleştiriyor...")
            ...
            try:
                ...
                try:
                    ...
                    if self.current_model_name != self.FALLBACK_MODEL:
                        err_msg = f"API YANIT HATASI (Bos Icerik/Block): {str(ve)[:50]}. YEDEK MODELE GECILIYOR..."
                        print(err_msg)
                        ...
                        err_msg = f"API YANIT HATASI (YEDEK MODEL DE BLOKE OLDU): {str(ve)[:50]}. 10s bekleyip tekrar deniyor..."
                        print(err_msg)
                        if progress_callback: progress_callback(err_msg)
                        time.sleep(10)
                        continue
                ...
                err_str = str(e).lower()
                if "api key" in err_str or "expired" in err_str or "invalid" in err_str:
                    err_msg = f"API KEY ERROR: Rotating key... {str(e)[:50]}"
                    print(err_msg)
                    if progress_callback: progress_callback(err_msg)
